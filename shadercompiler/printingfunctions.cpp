...
static void PrintShaderInputCPPFile(const ShaderInputs& inputs)
{
    ...
    const std::string outputDir = g_GlobalDirs.m_ShadersTmpCPPShaderInputsAutogenDir + inputs.m_Name.c_str() + ".h";
...
void PrintAutogenFileForShaderPermutationStructs(const Shader& shader)
{
    ...
    for (uint32_t i = 0; i < GfxShaderType_Count; ++i)
    {
        ...
        const std::string shaderTypeStr = EnumToString((GfxShaderType)i);

        generatedString += StringFormat("struct %s_%s {\n", shaderTypeStr.c_str(), shader.m_Name.c_str());
...
void PrintAutogenByteCodeHeadersFile(const concurrency::concurrent_vector<Shader>& allShaders)
{
    ...
    //    generatedString += "{\n\n";
    //
    //    for (const ShaderCompileJob& shaderJob : g_GlobalDirs.m_AllShaderCompileJobs)
    //    {
