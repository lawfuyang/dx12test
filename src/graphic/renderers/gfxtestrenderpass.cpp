...
static bool gs_ShowIMGUIWindow = false;
static float gs_DirLightAngle = 0.0f;
...
void GfxTestRenderPass::Initialize()
{
    ...
    g_IMGUIManager.RegisterTopMenu("Graphic", "GfxTestRenderPass", &gs_ShowIMGUIWindow);
...
void GfxTestRenderPass::PopulateCommandList()
{
    ...
    const bbeQuaternion rotQ = bbeQuaternion::CreateFromAxisAngle({ 0.0f, 1.0f, 0.0f }, ConvertToRadians(gs_DirLightAngle));
    const bbeVector3 dirLight = bbeVector3::Transform({ 0.577f, 0.577f , 0.577f }, rotQ);

    AutoGenerated::PerFrameConsts consts;
    consts.m_ViewProjMatrix = g_GfxView.GetViewProjMatrix();
    consts.m_SceneLightDir = dirLight;
...
{
    if (!gs_ShowIMGUIWindow)
        return;

    ScopedIMGUIWindow window{ "GfxTestRenderPass" };

    ImGui::SliderFloat("Dir Light Angle", &gs_DirLightAngle, 0.0f, 360.0f);
}
